...
String getBaseAmt = "SELECT SAR.product_key FROM `users` AS U, `series_allocation_record` AS SAR, `bid_record` AS BR , `users_personal_info` AS UPI "+
" WHERE U.user_id = SAR.user_id and SAR.order_id = BR.order_id and U.personal_info_id = UPI.personal_info_id  and SAR.status = 1 and U.user_id = '"+userId+"'";
...
<tr valign="top" >
	...
	<td>
		<fieldset style="border-color: #e2e2e2">
			<legend style="font-size: 18pt; color: #123456; font-weight: bold; font-family: arail">Bid Status</legend>
			<br/>
			<form name="bidStatus" action="BidStatusShow" method="post">
			...
			if(null != resRegn && resRegn.first()) {
				int cnt = 0;
				do {
					productKey = resRegn.getString("SAR.product_key");
					if (cnt%2==0){
			%>
					...
					<% } %>
						<td style="color: #123456; font-weight: bold;"><span style="color: #505050; font-size:10px; font-weight: bold;">Product Key : </span> <%=productKey %></td>
						<td></td>
						<td align="right" >
							<input type="hidden" name="productKey" id="productKey" value="">
							<input type="submit" style="height: 30px;width: 80px;border-radius:7px;background-color: #123456; color: white;" name="showBid" id="showBid" value="Show Bid" onclick="javascript:document.getElementById('productKey').value='<%=productKey  %>';">
						</td>
					</tr>
			<%
				cnt++;
